...
class FortiSandboxProvider(BinaryAnalysisProvider):
    def __init__(self, name, virustotal_api_token, url=None, rescan_window=None,log_level=None,submit_full_binaries=None):
        ...
        self.virustotal_analysis = FortiSandboxAnalysisClient(api_token=virustotal_api_token, session=session,log_level=log_level)
        self.url = url
...
class VirusTotalConnector(DetonationDaemon):
    ...
    def get_metadata(self):
        return cbint.utils.feed.generate_feed(self.name, summary="VirusTotal harnesses the power of over fifty-five Anti-Virus vendors to identify suspicious binaries.",
                                              tech_data="A VirusTotal private API key is required to use this feed. There are no requirements to share any data with Carbon Black to use this feed. However, binaries may be shared with fortisandbox.",
