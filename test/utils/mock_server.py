...
def get_mocked_server(app=None):
    ...
    mock_fortisandbox_server = MockFortisandboxServer()
    ...
    def binary_search_endpoint():
        ...
        return Response(response=json.dumps(mock_fortisandbox_server.get_binary_search(query_string, rows, start)),
                        mimetype='application/json')

    @flask_server.route('/api/v1/binary/<md5sum>/summary')
    def get_binary_summary(md5sum):
        binary_data = json.dumps(mock_fortisandbox_server.get_binary_summary())
    ...
    def get_binary(md5sum):
        binary_data = mock_fortisandbox_server.get_binary()
...
class MockFortisandboxServer(object):
    ...
    def get_fortisandbox_analysis_submission(self):
        return {'ver': '2.1', 'id': 11, 'result': {'url': '/alert/ondemand/submit-file', 'status': {'message': 'OK', 'code': 0}, 'data': {'msg': 'File was submitted successfully', 'sid': '5976427220022346030', 'error': ''}}}

    def handle_fortisandbox_analysis_submission(self, request, context):
        return self.get_fortisandbox_analysis_submission()

    def handle_fortisandbox_analysis_task_get_result(self, request, context):
        return self.get_fortisandbox_analysis_task_result()
